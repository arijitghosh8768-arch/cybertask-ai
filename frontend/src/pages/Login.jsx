...
import { Mail, Lock, LogIn, Cpu, Eye, EyeOff } from 'lucide-react';
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.msg || 'Invalid login details');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-950 px-4">
      ...
      <div className="relative w-full max-w-md glass-panel p-8 rounded-2xl shadow-2xl border border-slate-800 z-10 transition-all duration-300">
        <div className="text-center mb-8">
          <div className="inline-flex p-3 bg-cyan-500/10 rounded-xl mb-3 border border-cyan-500/20 text-cyan-400">
          ...
          <p className="text-sm text-slate-400 mt-2">Access your tactical objective neural network</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold uppercase tracking-widest text-slate-950 bg-gradient-to-r from-cyan-400 to-fuchsia-500 hover:from-cyan-300 hover:to-fuchsia-400 active:scale-95 transition-all duration-150 shadow-neon-blue cursor-pointer"
          >
            {loading ? (
